...
public class ChatClient {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    ManagedChannel channel = ManagedChannelBuilder
        ...
        .usePlaintext()
        .build();
    ChatGrpc.ChatStub stub = ChatGrpc.newStub(channel); //异步stub
    ChatGrpc.ChatStub stubSyncBlocking = ChatGrpc.newStub(channel); //同步阻塞stub
    ...
    ClientResponseObserver<HelloRequest, HelloReply> clientResponseObserver =
        new ClientResponseObserver<HelloRequest, HelloReply>() { //匿名类(new interface{})
              ...
              public void run() {
                ...
                while (requestStream.isReady()) {
                  if (iterator.hasNext()) {
                      ...
                      logger.info("--> " + name);
                      HelloRequest request = HelloRequest.newBuilder().setUserId(name).build();
          ...
          @Override
          public void onNext(HelloReply value) { //回调方法
    ...
    //注册
        ...
        logger.info("start register client:" + userId);
        HelloRequest request = HelloRequest.newBuilder().setName(userId).build();
        HelloReply response;
        try {
          response = stubSyncBlocking.registerClient(request); //同步阻塞调用远程服务 就和本地调用方法一样 //还有一种是异步 基于流观察者StreamObserver(服务器和客户端) 会回调onNext()方法
    ...
    stub.sendMsg(clientResponseObserver); //异步调用
