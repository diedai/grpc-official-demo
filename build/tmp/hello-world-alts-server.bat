...
@rem ##########################################################################
@rem
@rem  hello-world-alts-server startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and HELLO_WORLD_ALTS_SERVER_OPTS to pass JVM options to this script.
...
@rem Execute hello-world-alts-server
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %HELLO_WORLD_ALTS_SERVER_OPTS%  -classpath "%CLASSPATH%" io.grpc.examples.alts.HelloWorldAltsServer %CMD_LINE_ARGS%
...
:fail
rem Set variable HELLO_WORLD_ALTS_SERVER_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%HELLO_WORLD_ALTS_SERVER_EXIT_CONSOLE%" exit 1
exit /b 1
